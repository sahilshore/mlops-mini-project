...
# Split raw text data FIRST to prevent data leakage
    ...
    for algo_name, algorithm in algorithms.items():
        for vec_name, vectorizer in vectorizers.items():
            with mlflow.start_run(run_name=f"{algo_name} with {vec_name}", nested=True) as child_run:
                ...
                # 1. Create a Pipeline bundling the vectorizer and the algorithm
                model = Pipeline([
                ...
                # Model training (fits vectorizer on training data, then trains classifier)
                ...
                # 2. Infer signature for raw text input
                signature = infer_signature(X_train.head().to_frame(), model.predict(X_train.head().to_frame()))
                
                # 3. Log model with trusted types (required for your tree-based algorithms)
                # 3. Log model with trusted types (updated for XGBoost)
